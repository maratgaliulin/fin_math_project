...
def draw_do(df:pd.DataFrame, d_o: pd.DateOffset(), start_date, fig_sz, vol, type): # вспомогательная функция, отрисовывающая график в зависимости от значения type_graph (см. условия if/else ниже)
          try:
            ...
            signal_low = cumsum_true_false(df['Low'].loc[stdt:end_date], df['low_vol_cumsum_%_TrueFalse'].loc[stdt:end_date])
            signal_between = cumsum_true_false(df['High'].loc[stdt:end_date], df['between_vol_cumsum_%_TrueFalse'].loc[stdt:end_date])
            signal_high = cumsum_true_false(df['High'].loc[stdt:end_date], df['high_vol_cumsum_%_TrueFalse'].loc[stdt:end_date])
            ...
            apds = [
                ...
                mpf.make_addplot(signal_low,type='scatter',markersize=50,marker='^', color='#7285f7'),
                mpf.make_addplot(signal_between,type='scatter',markersize=50,marker='^', color='#0cf744'),
                mpf.make_addplot(signal_high,type='scatter',markersize=50,marker='^', color='#0cbff7'),
