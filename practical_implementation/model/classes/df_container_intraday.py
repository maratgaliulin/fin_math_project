...
class dfContainerIntraDay:
    def __init__(self, df_path):
        ...
        self.curr_df.columns = ['Date', 'Time', 'Open', 'High', 'Low', 'Close', 'Vol']
        self.curr_df[['Open', 'High', 'Low', 'Close', 'Vol']] = self.curr_df[['Open', 'High', 'Low', 'Close', 'Vol']].astype('float32')
    ...
    def draw_graph(self, end_date:str, start_date='', fig_sz = (8,8)):
        if(start_date != ''):
            mpf.plot(self.curr_df.loc[pd.to_datetime(start_date):pd.to_datetime(end_date)], type='candle',mav=(3,6,9), figsize=fig_sz)
            plt.show()
