...
def date_time_index_maker(dfr:pd.DataFrame): # функция по созданию строки типа "2023-12-14 00:00:00" из интовых данных столбцов <DATE> и <TIME>
    ...
    return pd.to_datetime(dfr['Date'][2:4] + '-' + dfr['Date'][4:] + '-' + dfr['Date'][0:2] + ' ' + dfr['Time'][0:2] + ':' + dfr['Time'][2:4] + ':' + dfr['Time'][4:])
...
class dfContainerIntraDay:
    ...
    def draw_graph(self, start_date='2022-12-01', fig_sz = (8,8), vol=False, type_graph='day'):  # здесь добавляется переменная vol. Если ее значение True, то будет отрисовываться торговый объем. По умолчанию False.
    ...
    # Условный оператор в зависимости от переменной type_graph:
        if type_graph == 'day':
          draw_do(pd.DateOffset(1), start_date, fig_sz, vol, 'candle')
        elif type_graph == 'week':
          draw_do(pd.DateOffset(weeks=1), start_date, fig_sz, vol, 'candle')
        elif type_graph == 'month':
          draw_do(pd.DateOffset(months=1), start_date, fig_sz, vol, 'line')
        else:
          print('Диапазон данных слишком велик либо введен неверный формат данных')
